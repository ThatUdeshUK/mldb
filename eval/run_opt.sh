...
export RUNS=3
export BATCH_SIZES=(1 4 16 64 256 1024) # 2048 4096) #  4096
export ONNX_INTRA_THREADS=(16 32 64) # 64 128 256 512 1024 4096
...
PRAGMA profile_output='/homes/ukumaras/Projects/mldb/eval/llm_performance/opt_vec_onnx_distilbert_imdb_n_${CARDINALITY}_lmt_${LLM_MAX_TOKENS}_bs_${BATCH_SIZE}_run_${RUN}_intratc_${INTRA_THREAD}.json';
...
SELECT * FROM PREDICT ('/homes/ukumaras/scratch/Models/distilbert-base-uncased-finetuned-sst-2-english-onnx', (FROM imdb WHERE stage == 'test' LIMIT ${CARDINALITY})) BY (text) WITH (neg FLOAT, pos FLOAT);
.q
QUERIES
            done
        done
